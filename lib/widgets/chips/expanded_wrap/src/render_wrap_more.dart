...
class _RunMetrics {
  ...
  static void _setChildNextDrop(bool needPaintDropChild, RenderBox child) {
    (child.parentData! as WrapMoreParentData).needPaintDropChild = needPaintDropChild;
  }
  ...
  List<_RunMetrics>? tryAddingNewChild(
    ...
    final double totalSpacing = separateSize == null ? spacing : separateSize.mainAxisExtent;
    final double childTotal = axisSize.mainAxisExtent + childSize.mainAxisExtent + totalSpacing;
          ...
          if (!hasNextChild) {
            ...
            if (needsNewRun) {
              ...
              if (currentLine > minLines) {
                // [child]不会在新的一行
                breakDropLine =
                    childTotal + drop.spacing + drop.axisSize.mainAxisExtent - maxMainExtent >
                        precisionErrorTolerance;
                resultDropChild = drop.child;
              }
            }
            ...
            if (currentLine >= maxLines) {
              if (needsNewRun) {
                ...
                if (childTotal + drop.spacing + drop.axisSize.mainAxisExtent - maxMainExtent >
                    precisionErrorTolerance) {
    ...
    if (needsNewRun) {
      ...
      if (child == resultDropChild) {
        ...
        if (breakDropLine) {
          runMetricsList.add(_RunMetrics(resultDropChild, dropLayout!.axisSize, resultDropChild));
        } else {
      ...
      if (child == resultDropChild) {
        ...
        if (resultDropChild != null) {
          _setChildNextDrop(true, child);
          if (breakDropLine) {
            return [_RunMetrics(resultDropChild, dropLayout!.axisSize, resultDropChild)];
          }
          dropChild = resultDropChild;
          _addDropChild(this, dropLayout!.axisSize, dropLayout.spacing);
        }
  ...
  static void _addDropChild(
    ...
    if (runMetrics.dropChild != null) {
      runMetrics.axisSize +=
          dropChildSize + _AxisSize(mainAxisExtent: spacing, crossAxisExtent: 0.0);
...
class RenderWrapMore extends RenderBox
  ...
  AxisDirection get _nearRealDirection => switch (_nearDirection) {
        AxisDirection.left =>
          textDirection == TextDirection.rtl ? AxisDirection.right : AxisDirection.left,
        AxisDirection.right =>
          textDirection == TextDirection.rtl ? AxisDirection.left : AxisDirection.right,
        AxisDirection.up =>
          verticalDirection == VerticalDirection.up ? AxisDirection.down : AxisDirection.up,
        AxisDirection.down =>
          verticalDirection == VerticalDirection.up ? AxisDirection.up : AxisDirection.down,
      };
  ...
  void visitChildren(RenderObjectVisitor visitor) {
    ...
    while (child != null) {
      visitor(child);
      final WrapMoreParentData childParentData = child.parentData! as WrapMoreParentData;
  ...
  bool get _debugHasNecessaryDirections {
    ...
    if (alignment == WrapMoreAlignment.start || alignment == WrapMoreAlignment.end) {
      switch (direction) {
        case Axis.horizontal:
          assert(textDirection != null,
    ...
    if (runAlignment == WrapMoreAlignment.start || runAlignment == WrapMoreAlignment.end) {
      switch (direction) {
        case Axis.horizontal:
          break;
        case Axis.vertical:
    ...
    switch (direction) {
      case Axis.horizontal:
        ...
        if (dropRenderBox != null) {
          width = math.max(width, dropRenderBox?.getMinIntrinsicWidth(double.infinity) ?? 0.0);
        }
        if (nearRenderBox != null) {
          width = math.max(width, nearRenderBox?.getMinIntrinsicWidth(double.infinity) ?? 0.0);
        }
        if (separateRenderBox != null) {
          width = math.max(width, separateRenderBox?.getMinIntrinsicWidth(double.infinity) ?? 0.0);
        }
        return width;
      case Axis.vertical:
  ...
  double computeMaxIntrinsicWidth(double height) {
    switch (direction) {
      case Axis.horizontal:
        ...
        // 没处理隐藏逻辑，隐藏逻辑需要在[layout]之后才知道[固有大小的计算不需要处理]
        if (dropRenderBox != null) {
          width += (dropRenderBox?.getMaxIntrinsicWidth(double.infinity) ?? 0.0);
        }
        if (nearRenderBox != null) {
          width += (nearRenderBox?.getMaxIntrinsicWidth(double.infinity) ?? 0.0);
        }
        if (separateRenderBox != null) {
          width += separateRenderBox?.getMaxIntrinsicWidth(double.infinity) ?? 0.0;
        }
        return width;
      case Axis.vertical:
        return getDryLayout(BoxConstraints(maxHeight: height)).width;
  ...
  double computeMinIntrinsicHeight(double width) {
    switch (direction) {
      ...
      case Axis.vertical:
        ...
        // 没处理隐藏逻辑，隐藏逻辑需要在[layout]之后才知道[固有大小的计算不需要处理]
        if (dropRenderBox != null) {
          height = math.max(height, dropRenderBox?.getMinIntrinsicHeight(double.infinity) ?? 0.0);
        }
        if (nearRenderBox != null) {
          height = math.max(height, nearRenderBox?.getMinIntrinsicHeight(double.infinity) ?? 0.0);
        }
        if (separateRenderBox != null) {
          height +=
              math.max(height, separateRenderBox?.getMinIntrinsicHeight(double.infinity) ?? 0.0);
        }
        return height;
    }
  }
  ...
  double computeMaxIntrinsicHeight(double width) {
    switch (direction) {
      ...
      case Axis.vertical:
        ...
        // 没处理隐藏逻辑，隐藏逻辑需要在[layout]之后才知道[固有大小的计算不需要处理]
        if (dropRenderBox != null) {
          height += (dropRenderBox?.getMaxIntrinsicHeight(double.infinity) ?? 0.0);
        }
        if (nearRenderBox != null) {
          height += (nearRenderBox?.getMaxIntrinsicHeight(double.infinity) ?? 0.0);
        }
        if (separateRenderBox != null) {
          height += (separateRenderBox?.getMaxIntrinsicHeight(double.infinity) ?? 0.0);
        }
        return height;
    }
  }
  ...
  double? computeDryBaseline(covariant BoxConstraints constraints, TextBaseline baseline) {
    ...
    void findHighestBaseline(Offset offset, RenderBox child) {
      baselineOffset = baselineOffset
          .minOf(BaselineOffset(child.getDryBaseline(childConstraints, baseline)) + offset.dy);
    }

    Size getChildSize(RenderBox child) => child.getDryLayout(childConstraints);
    _positionChildren(runMetrics, childrenAxisSize, containerAxisSize, nearSize, separateSize,
  ...
  static void _resetParentData(RenderBox child) {
    final WrapMoreParentData parentData = child.parentData! as WrapMoreParentData;
  ...
  // 是否方向相同
  bool get _sameNearDirection => axisDirectionToAxis(_nearDirection) == direction;
  ...
  void performLayout() {
    ...
    _AxisSize containerAxisSize = childrenAxisSize.applyConstraints(constraints, direction);
    ...
    final _AxisSize freeAxisSize = containerAxisSize -
        ...
        (nearSize != null
            ? _AxisSize(
                mainAxisExtent: sameNearDirection ? (nearSize.mainAxisExtent + nearSpacing) : 0.0,
                crossAxisExtent: sameNearDirection ? 0.0 : (nearSize.crossAxisExtent + nearSpacing),
              )
            : _AxisSize.empty);
    _hasVisualOverflow = freeAxisSize.mainAxisExtent < 0.0 || freeAxisSize.crossAxisExtent < 0.0;
    _positionChildren(runMetrics, freeAxisSize, containerAxisSize, nearSize, separateSize,
  ...
  dynamic _layoutRuns(
    ...
    for (RenderBox? child = firstChild; child != null; child = nextChild) {
      ...
      final RenderBox? runDropChild = newRun?.dropChild ?? currentRun?.dropChild;
      if (newRun != null) {
            ...
            if (previousRun.dropChild == null) {
              if (dropLayout.layout().hasSize) {
                ...
                if (lookupMore) {
                  ...
                  currentRun = _RunMetrics(dropLayout.child, dropLayout.axisSize, dropLayout.child);
  ...
  (
    ...
    final (BoxConstraints childConstraints, double mainAxisLimit, double crossAxisLimit) =
        switch (direction) {
      Axis.horizontal => (
          BoxConstraints(maxWidth: constraints.maxWidth),
          constraints.maxWidth,
    ...
    Size tryLayoutChild(RenderBox child, [BoxConstraints? myChildConstraints]) {
      final Size size = layoutChildren.putIfAbsent(
          child, () => layoutChild(child, myChildConstraints ?? childConstraints));
      if (nearSize == null || !sameNearDirection) return size;
      final (
        double maxMainSize,
        BoxConstraints newChildConstraints,
      ) = switch (direction) {
        Axis.horizontal => (size.width, BoxConstraints(maxWidth: tmpMainAxisLimit)),
        Axis.vertical => (size.width, BoxConstraints(maxHeight: tmpMainAxisLimit)),
      };
    ...
    if (nearRenderBox != null) {
      ...
      final bool isStretch = !sameNearDirection && nearAlignment == WrapMoreNearAlignment.stretch;
    ...
    if (nearLayout != null && nearLayout.hasSize) {
      ...
      if (isShowNearChild) {
        ...
        if (sameNearDirection) {
          tmpMainAxisLimit =
              math.max(0.0, mainAxisLimit - (nearSize?.mainAxisExtent ?? 0.0) - nearSpacing);
        }
      }
    }
    ...
    tmpChildrenAxisSize += _AxisSize(mainAxisExtent: totalRunSpacing, crossAxisExtent: 0.0) +
    ...
    if (nearSize != null) {
      ...
      if (sameNearDirection) {
        if (isStretch || nearSize.mainAxisExtent > tmpChildrenAxisSize.crossAxisExtent) {
          // 此时[tmpChildrenAxisSize]的主轴和侧轴是反向的
          final double nearMaxMainLimit = mainAxisLimit - tmpChildrenAxisSize.crossAxisExtent;
        ...
        tmpChildrenAxisSize += _AxisSize(mainAxisExtent: nearSpacing, crossAxisExtent: 0.0);
      }

      // 存在[nearChild]时，需要进一步计算[tmpChildrenAxisSize]
      final _AxisSize nearTotalSize =
          _AxisSize(mainAxisExtent: nearSpacing, crossAxisExtent: 0.0) + nearSize;
      if (sameNearDirection) {
        tmpChildrenAxisSize = tmpChildrenAxisSize.flipped + nearTotalSize;
      } else {
        tmpChildrenAxisSize = (tmpChildrenAxisSize + nearTotalSize.flipped).flipped;
      }
    } else {
      tmpChildrenAxisSize = tmpChildrenAxisSize.flipped;
    }
    return (tmpChildrenAxisSize, onlyChildrenAxisSize, runMetrics, nearSize, separateSize);
  }
  ...
  (
    ...
    if (nearAxis == direction) {
      final bool isStartPosition =
          nearDirection == AxisDirection.left || nearDirection == AxisDirection.up;
      final double alignment =
          flipCrossAxis ? nearAlignment.flipped.alignment : nearAlignment.alignment;
      final double mainOffset = isStartPosition
          ? 0.0
          : math.max(0.0, containerAxisSize.mainAxisExtent - nearAxisSize.mainAxisExtent);
      final double crossOffset =
          (containerAxisSize.crossAxisExtent - nearAxisSize.crossAxisExtent) * alignment;
      ...
      final bool isStarCrossPosition =
          nearDirection == AxisDirection.right || nearDirection == AxisDirection.down;
      final double alignment =
          flipMainAxis ? nearAlignment.flipped.alignment : nearAlignment.alignment;
      final double mainOffset =
          (containerAxisSize.mainAxisExtent - nearAxisSize.mainAxisExtent) * alignment;
      ...
      needChildrenCrossOffset =
          isStarCrossPosition ? 0.0 : (nearAxisSize.crossAxisExtent + nearSpacing);
    }
    positionChild(offset, nearRenderBox!);
    return (needChildrenMainOffset, needChildrenCrossOffset, needMainSize, needCrossSize);
  }
  ...
  void _positionChildren(
    ...
    final (
      ...
      double needMainSize,
      double needCrossSize
    ) = _positionNearChild(containerAxisSize, nearSize, flipMainAxis, flipCrossAxis, positionChild);

    final WrapMoreCrossAlignment effectiveCrossAlignment =
        flipCrossAxis ? crossAxisAlignment.flipped : crossAxisAlignment;
    final (double runLeadingSpace, double runBetweenSpace) = runAlignment.distributeSpace(
      crossAxisFreeSpace,
      runSpacing,
      runMetrics.length,
      flipCrossAxis,
    ...
    for (final _RunMetrics run in runs) {
      ...
      final bool hasDropChild = run.dropChild != null && run.dropChild == dropRenderBox;
      final int childCountWithoutDropChild = hasDropChild ? childCount - 1 : childCount;

      final double mainAxisFreeSpace = math.max(
          0.0, containerAxisSize.mainAxisExtent - run.axisSize.mainAxisExtent - needMainSize);
      final (double childLeadingSpace, double childBetweenSpace) = alignment.distributeSpace(
          mainAxisFreeSpace, totalSpacing, math.max(1, childCountWithoutDropChild), flipMainAxis);
      ...
      final positionedDropChild = hasDropChild
          ? (double mainAxisOffset) {
              ...
              ) = _AxisSize.fromSize(size: getChildSize(dropChild), direction: direction);
              final double childCrossAxisOffset =
                  effectiveCrossAlignment.alignment * (runCrossAxisExtent - childCrossAxisExtent);
              if (!flipMainAxis) {
              ...
              positionChild(
                  _getOffset(mainAxisOffset, runCrossAxisOffset + childCrossAxisOffset), dropChild);
              if (flipMainAxis) {
                mainAxisOffset += childMainAxisExtent +
                    (dropChildSpacing == null ? childBetweenSpace : dropChildSpacing!);
              } else {
      ...
      for (RenderBox? child = run.leadingChild;
        ...
        final double childCrossAxisOffset =
            effectiveCrossAlignment.alignment * (runCrossAxisExtent - childCrossAxisExtent);
        positionChild(
            _getOffset(childMainAxisOffset, runCrossAxisOffset + childCrossAxisOffset), child);
        final WrapMoreParentData parentData = child.parentData as WrapMoreParentData;
        if (parentData.hasNextSeparate) {
          // Next step positioned separateChild
          final double separateChildCrossAxisOffset =
              effectiveCrossAlignment.alignment * (runCrossAxisExtent - separateCrossAxisExtent);
  ...
  bool _hitTestChildren(BoxHitTestResult result, {required Offset position}) {
    ...
    for (int i = length - 1; i >= 0; i--) {
      ...
      if (child == separateRenderBox) {
        ...
        final WrapMoreParentData preChildParentData = preChild.parentData! as WrapMoreParentData;
  ...
  void _paint(PaintingContext context, Offset offset) {
    ...
    while (child != null) {
      ...
      if (childParentData.needPaintDropChild) {
        ...
        if (dropRenderBox != null) {
          ...
          context.paintChild(dropRenderBox, dropChildParentData.offset + offset);
        }
        ...
        if (separateRenderBox != null) {
          context.paintChild(separateRenderBox, childParentData.nextSeparateOffset + offset);
        }
      }
      child = childParentData.nextSibling;
    }
    ...
    if (nearRenderBox case final child?) {
      final WrapMoreParentData nearParentData = child.parentData! as WrapMoreParentData;
