...
class WrapMoreElement extends RenderObjectElement {
  ...
  Element? get dropElement => _forgottenChildren.contains(_dropElement) ? null : _dropElement;
  Element? _dropElement;

  Element? get nearElement => _forgottenChildren.contains(_nearElement) ? null : _nearElement;
  ...
  void moveRenderObjectChild(
      RenderObject child, IndexedSlot<Element?> oldSlot, IndexedSlot<Element?> newSlot) {
    final ContainerRenderObjectMixin<RenderObject, ContainerParentDataMixin<RenderObject>>
        renderObject = this.renderObject;
  ...
  void removeRenderObjectChild(RenderObject child, Object? slot) {
    final ContainerRenderObjectMixin<RenderObject, ContainerParentDataMixin<RenderObject>>
        renderObject = this.renderObject; //RenderWrapMore renderObject
    assert(child.parent == renderObject);
    if (slot == dropElementSlot) {
      final RenderWrapMore parent = renderObject as RenderWrapMore;
  ...
  void visitChildren(ElementVisitor visitor) {
    ...
    if (_separateElement != null && !_forgottenChildren.contains(_separateElement)) {
  ...
  void mount(Element? parent, Object? newSlot) {
    ...
    for (int i = 0; i < children.length; i += 1) {
      final Element newChild =
          inflateWidget(wrapMore.children[i], IndexedSlot<Element?>(i, previousChild));
      children[i] = newChild;
